...
export default function ItemTypeClient({ initialItemTypes }: ItemTypeClientProps) {
	const router = useRouter();
	const [itemTypes, setItemTypes] = useState<ItemTypeDTO[]>(initialItemTypes);
	const [open, setOpen] = useState(false);
	const [editingItemType, setEditingItemType] = useState<ItemTypeDTO | null>(null);
			...
			>
				...
				<div className="flex items-center gap-3">
					...
					<GlassButton
						...
						className="shadow-xl hover:scale-105 transition-all duration-300 px-6 py-4 rounded-2xl font-black uppercase tracking-wider flex items-center gap-2"
					>
						{selectedIds.length === itemTypes.length ? "Batal Semua" : "Pilih Semua"}
					</GlassButton>
					<GlassButton
						onClick={() => handleOpen()}
