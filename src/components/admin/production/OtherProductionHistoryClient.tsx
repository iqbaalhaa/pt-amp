...
import {
  getProduksiLainnyaHistory,
  deleteProduksiLainnya,
} from "@/actions/produksi-lainnya-actions";
...
type HistoryItem = {
  ...
  notes: string | null;
  totalBiaya: number;
  items: Array<{ namaPekerjaan?: string; [k: string]: any }>;
};
...
export default function OtherProductionHistoryClient() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getProduksiLainnyaHistory();
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      const pekerjaan = (row.items || [])
        .map((i) => i?.namaPekerjaan || "")
        .join(", ")
        .toLowerCase();

      return tanggal.includes(q) || notes.includes(q) || pekerjaan.includes(q);
    });
  }, [data, query]);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await deleteProduksiLainnya(id);
      if (res?.success) {
        setToastState({
          open: true,
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Riwayat Produksi Lainnya"
        subtitle="Daftar input produksi lainnya yang tersimpan (bisa cari & hapus)."
        actions={
          <>
            <Link href="/admin/produksi-lainnya">
      ...
      {/* CONTAINER: transparan + border doang */}
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="relative w-full md:max-w-md">
            ...
            <input
              ...
              placeholder="Cari tanggal / catatan / pekerjaan…"
            />
          </div>
            ...
            <tbody>
              {loading ? (
                ...
                filtered.map((row, idx) => {
                  const pekerjaanPreview = (row.items || [])
                    .map((i) => i?.namaPekerjaan)
                    .filter(Boolean)
                    .join(", ");
