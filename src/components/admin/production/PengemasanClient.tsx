...
export default function PengemasanClient() {
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== "undefined") {
        ...
        if (raw) {
          const parsed = JSON.parse(raw) as { pengemasanPerBungkus?: number };
          if (typeof parsed.pengemasanPerBungkus === "number") {
            setUpahPerBungkus(parsed.pengemasanPerBungkus);
          }
        }
      }
    } catch {
  ...
  const handleDownloadPdf = async () => {
    ...
    if (logo) {
      ...
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(9);
      pdf.text("REKAP PENGEMASAN", pageW - margin, y + 5, {
        align: "right",
      });
    ...
    pdf.save(`nota-pengemasan-${date || "draft"}.pdf`);
  };
