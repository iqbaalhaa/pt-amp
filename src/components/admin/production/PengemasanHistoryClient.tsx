...
export default function PengemasanHistoryClient() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getPengemasanHistory();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await deletePengemasan(id);
      if (res?.success) {
        setToastState({
          open: true,
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        subtitle="Daftar input pengemasan yang tersimpan (bisa cari & hapus)."
        actions={
          <>
            <Link href="/admin/pengemasan">
