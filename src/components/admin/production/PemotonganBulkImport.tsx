...
export default function PemotonganBulkImport() {
  const [open, setOpen] = useState(false);
  const [text, setText] = useState("");
  const [upahPerKg, setUpahPerKg] = useState("100");
  ...
  const parsedData = useMemo(() => {
    ...
    // Format: Date \t Name \t Qty
    const isTabSeparated = lines.some((l) => l.includes("\t"));

    let rawItems: { date: string; name: string; qty: string }[] = [];
    ...
    // Group by Date
    const groups: Record<string, PemotonganItemInput[]> = {};
    ...
    // Convert to PemotonganInput array
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await bulkCreatePemotongan(parsedData);
      if (res.success) {
        setSuccessMsg(
  ...
  return (
    <>
      ...
      <SafeModal
        ...
        title="Import Data Pemotongan (Bulk)"
      >
