...
export default function PengikisanHistoryClient() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getPengikisanHistory();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await deletePengikisan(id);
      if (res?.success) {
        setToastState({
          open: true,
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Riwayat Pengikisan"
        subtitle="Daftar input pengikisan yang tersimpan (bisa cari & hapus)."
        actions={
          <>
            <Link href="/admin/pengikisan">
