...
export default function PenjemuranHistoryClient() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getPenjemuranHistory();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await deletePenjemuran(id);
      if (res?.success) {
        setToastState({
          open: true,
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Riwayat Penjemuran"
        subtitle="Daftar input penjemuran yang tersimpan (bisa cari & hapus)."
        actions={
          <>
            <Link href="/admin/penjemuran">
