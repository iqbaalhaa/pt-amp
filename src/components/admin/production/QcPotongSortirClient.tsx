...
export default function QcPotongSortirClient() {
    ...
    try {
      if (typeof window !== "undefined") {
        ...
        if (raw) {
          ...
          // Try to find relevant keys, or fallback to generic defaults
          const perHari =
            parsed.qcPotongSortirPerHari ||
            parsed.penjemuranPerHari ||
            parsed.defaultPerHari;
          const perJam =
            parsed.qcPotongSortirLemburPerJam ||
            parsed.penjemuranLemburPerJam ||
            parsed.defaultLemburPerJam;

          if (typeof perHari === "number") {
            setUpahPerHari(perHari);
          }
          if (typeof perJam === "number") {
            setUpahLemburPerJam(perJam);
          }
        }
      }
    } catch {
  ...
  const activeRows = useMemo(
    () => rows.filter((r) => r.nama || r.lemburJam > 0),
    [rows]
  );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const validRows = rows.filter((r) => r.nama);

    if (validRows.length === 0) {
      alert("Mohon isi minimal satu baris data dengan nama pekerja");
      return;
    }
  ...
  const handleDownloadPdf = async () => {
    ...
    rowsForPrint.forEach((row, idx) => {
      ...
      pdf.text(row.hadir === "HADIR" ? "Ya" : "Tidak", colHariX, y);
              ...
              <thead className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-[var(--glass-border)]">
                <tr className="text-[11px] font-extrabold tracking-wide text-black/75 uppercase">
                  ...
                  <th className="px-3 py-3 text-center min-w-[140px]">
                    <span className="inline-flex flex-col items-center gap-0.5 justify-center">
