...
type ItemRow = {
	id: string; // internal client-side id
	itemTypeId: string;
	qty: string;
	unitPrice: string;
};
...
const PREVIEW_ASPECT = A5_H_MM / A5_W_MM;
const PREVIEW_MIN_HEIGHT_PX = PREVIEW_WIDTH_PX * PREVIEW_ASPECT;
	...
	const invoiceItems = useMemo(() => {
		return items
			.map((item) => {
				...
				const total = isFinite(q * p) ? q * p : 0;

				return {
					productName: it ? it.name : "-",
					qty: item.qty || "0",
					unit: it?.unit || "unit", // Use unit from ItemType or default
								...
								<table className="w-full text-[12px] text-left">
									...
									<tbody>
										{items.map((row, idx) => {
											...
											const selectedItem = activeItemTypes.find(
											...
											return (
												...
												>
													...
													<td className="px-3 py-2">
														<div
															className={cx(
																"w-full h-[38px] px-3 rounded-lg flex items-center text-[12px]",
																"border border-transparent bg-black/[0.03] text-black/60",
															)}
														>
															{selectedItem?.unit || "-"}
														</div>
													...
													<td className="px-2 py-2">
														<input
															...
															value={row.unitPrice}
															onChange={(e) =>
																updateItem(row.id, "unitPrice", e.target.value)
															}
															placeholder="0"
															className={cx(
