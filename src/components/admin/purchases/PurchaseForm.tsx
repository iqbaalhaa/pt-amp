...
export default function PurchaseForm({
  ...
  const [localSuppliers, setLocalSuppliers] =
    useState<SupplierDTO[]>(initialSuppliers);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    openConfirm({
      ...
      message: `Apakah Anda yakin ingin menyimpan transaksi pembelian ini dengan total ${formatRupiah(
        grandTotal
      )}?`,
      variant: "primary",
      confirmText: "Simpan",
      onConfirm: async () => {
        try {
          const payload = {
            supplier:
              localSuppliers.find((s) => s.id === supplierId)?.name || null,
            date,
            status,
            notes: notes || null,
          ...
          onChange={async (event, newValue) => {
            if (newValue && (newValue as any).inputValue) {
              ...
              try {
                ...
                setSnack({
                  open: true,
                  message: `Satuan "${newUnit.name}" berhasil ditambahkan`,
                  severity: "success",
                });
              } catch (err) {
                setSnack({
                  open: true,
                  message: "Gagal menambahkan satuan baru",
                  severity: "error",
                });
              } finally {
                ...
                onChange={async (event, newValue) => {
                  if (newValue && (newValue as any).inputValue) {
                    ...
                    try {
                      ...
                      setSnack({
                        open: true,
                        message: `Supplier "${newSup.name}" berhasil ditambahkan`,
                        severity: "success",
                      });
                    } catch (err) {
                      setSnack({
