        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl">
            ...
            <p className="text-zinc-400 text-lg md:text-xl leading-relaxed">
              Pilihan lengkap produk kulit manis dengan berbagai grade dan
              spesifikasi pemotongan. Diproses higienis untuk memenuhi standar
              ekspor.
            </p>
          </div>
        </div>
          ...
          {products.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.map((product) => (
                <div
                  key={product.id}
                  className="bg-white rounded-2xl overflow-hidden border border-zinc-200 hover:border-[var(--brand)] hover:shadow-xl transition-all duration-300 group flex flex-col h-full"
                >
                  <ProductCatalogImage
                    image={product.image}
                    name={product.name}
