...
import { getAboutPageData, getFeatureCards } from "@/actions/cms-actions";
import { DynamicIcon } from "@/components/DynamicIcon";

export default async function AboutPage() {
  const [aboutPage, featureCards] = await Promise.all([
    getAboutPageData(),
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <div className="max-w-4xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-8 text-zinc-900 tracking-tight">
              {aboutPage?.heroTitle || "Dedikasi untuk Kualitas & Keaslian Kulit Manis"}
            </h1>
            <p className="text-xl text-zinc-600 leading-relaxed max-w-3xl">
              {aboutPage?.heroDescription || "PT AMP berdiri dengan visi mengangkat potensi komoditas lokal ke pasar global. Kami menjembatani petani kulit manis dengan industri melalui proses pengolahan yang terstandarisasi, transparan, dan berkelanjutan."}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-center mb-24">
             <div className="order-2 md:order-1">
                <h2 className="text-3xl font-bold mb-6 text-zinc-900">{aboutPage?.mainTitle || "Siapa Kami?"}</h2>
                <div className="text-lg text-zinc-600 leading-relaxed mb-8 whitespace-pre-line">
                  {aboutPage?.mainDescription || "Kami adalah perusahaan pengolahan kulit manis (Cinnamomum burmannii) skala menengah yang berlokasi di Sumatera Barat. Fokus utama kami adalah pembelian bahan mentah langsung dari petani, melakukan pembersihan, pengikisan, penjemuran, dan pengemasan profesional.\n\nDengan dukungan tim ±10 tenaga kerja terampil, kami memproses bahan baku menjadi produk setengah jadi (Grade A, B, Broken) yang siap diekspor atau didistribusikan ke industri makanan dan farmasi lokal."}
                </div>
                
                <div className="flex flex-col gap-3">
                  {aboutPage?.points && aboutPage.points.length > 0 ? (
                    aboutPage.points.map((point) => (
                      <div key={point.id} className="flex items-center gap-3">
                        ...
                        <span className="text-zinc-700 font-medium">{point.text}</span>
                      </div>
                    ))
                  ) : (
                    <>
             ...
             <div className="order-1 md:order-2 bg-zinc-100 rounded-3xl overflow-hidden aspect-square relative">
                <img 
                   src={aboutPage?.mainImage || "https://images.unsplash.com/photo-1596483582103-605b7668636b?q=80&w=2070&auto=format&fit=crop"}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {featureCards.length > 0 ? featureCards.map((card) => (
              <div key={card.id} className="bg-zinc-50 rounded-2xl p-8 border border-zinc-100 hover:border-[var(--brand)]/30 transition-colors">
                ...
                <h3 className="text-xl font-bold mb-3 text-zinc-900">{card.title}</h3>
