...
import { getHomePageData, getHeroSlides, getFeatureCards } from "@/actions/cms-actions";
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <div className="max-w-3xl mb-12">
            ...
            <p className="text-lg text-zinc-600 leading-relaxed mb-6">
              {homePage?.aboutDescription || "Kami membeli kulit manis dari petani, melakukan pembersihan, pengikisan, penjemuran, dan pengemasan dalam ball 40–60 kg. Dengan tim ±10 orang, kami menjaga kualitas untuk memenuhi kebutuhan pasar lokal dan ekspor."}
            </p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {featureCards.length > 0 ? featureCards.map((card) => (
              <div key={card.id} className="bg-white rounded-xl border border-zinc-200 p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
                <div className="w-12 h-12 rounded-lg bg-red-50 flex items-center justify-center mb-6 text-brand group-hover:bg-red-100 transition-colors">
                  <DynamicIcon name={card.icon} className="w-6 h-6 text-[var(--brand)]" />
                </div>
                <h3 className="text-xl font-semibold mb-3 text-zinc-900">{card.title}</h3>
