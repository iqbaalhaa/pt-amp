...
import { HomeBlogImage } from "./HomeBlogImage";
...
export default async function HomePage() {
  ...
  const finalSlides =
    slides.length > 0 ? (slides as unknown as HeroSlide[]) : [];

  return (
    <div className="min-h-screen font-sans bg-zinc-50">
      <Navbar />

      <HeroCarousel slides={finalSlides} />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {featureCards.length > 0 ? (
              featureCards.map((card) => (
                <div
                  key={card.id}
                  className="bg-white rounded-xl border border-zinc-200 p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group"
                >
              ...
              // Fallback content if no feature cards found
              <>
                <div className="bg-white rounded-xl border border-zinc-200 p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
                  ...
                  <p className="text-zinc-600">
                    Kemitraan dengan petani untuk bahan baku berkualitas.
                  </p>
                </div>
                {/* ... other fallbacks ... */}
              </>
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-zinc-600">
                Produk kulit manis siap jual dalam berbagai grade.
              </p>
            </div>
            <Link
              href="/products"
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
            {!showStatic ? (
              products.map((product) => (
                <div
                  key={product.id}
                  className="group bg-white rounded-2xl border border-zinc-200 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                >
                  ...
                  <div className="p-6">
                    <div className="text-xl font-semibold mb-2 text-zinc-900">
                      {product.name}
                    </div>
      ...
      <section id="gallery" className="py-20 bg-white">
        ...
        <GalleryAlbums
          albums={albums}
          showTitle={false}
          className="py-0 min-h-0 bg-white"
        />
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-zinc-600">
                Berita dan artikel terbaru seputar kulit manis.
              </p>
            </div>
            <Link
              href="/blog"
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            {posts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group bg-white rounded-xl border border-zinc-200 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 block h-full flex flex-col"
              >
                <HomeBlogImage image={post.image} title={post.title} />
                <div className="p-6 flex flex-col flex-grow">
                  <div className="text-sm text-zinc-500 mb-2">{post.date}</div>
                  <h3
                    className="text-xl font-bold text-zinc-900 mb-3 group-hover:text-brand transition-colors"
                    style={{ color: "var(--brand)" }}
                  >
                    {post.title}
                  </h3>
                  <p className="text-zinc-600 line-clamp-3 text-sm flex-grow">
                    {post.summary}
                  </p>
                  <div
                    className="mt-4 text-brand font-medium text-sm flex items-center"
                    style={{ color: "var(--brand)" }}
                  >
                    Baca Selengkapnya{" "}
                    <svg
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <p className="text-lg text-zinc-600 mb-4 leading-relaxed">
                Apakah Anda petani yang ingin menjual hasil panen, atau pembeli
                yang mencari pasokan kulit manis berkualitas? Kami siap menjadi
                partner terpercaya Anda.
              </p>
              <Link
                href="/contact"
                className="inline-flex items-center text-brand font-medium hover:underline mb-8"
                style={{ color: "var(--brand)" }}
              >
