...
export async function GET(_req: Request, ctx: { params: Promise<{ model: string }> }) {
	...
	const items = await modelResult.delegate.findMany();
	return jsonResponse(items);
}

export async function POST(req: Request, ctx: { params: Promise<{ model: string }> }) {
	const authResult = await requireSuperadmin();
	if (!authResult.ok) return authResult.res;
	...
	const created = await modelResult.delegate.create({ data });
