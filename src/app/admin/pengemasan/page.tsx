...
export default function PengemasanPage() {
	...
	const [newBungkus, setNewBungkus] = useState<string>("");
	...
	const addRow = () => {
		setRows((prev) => {
			...
			return [...prev, { id: nextId, nama: "", bungkus: 0 }];
		});
	};
	...
	const addFromForm = () => {
		const bungkus = Number(newBungkus);
		const namaTrimmed = newNama.trim();

		setRows((prev) => {
			if (prev.length === 1 && prev[0].nama === "" && prev[0].bungkus === 0) {
				return [
					{
						...prev[0],
						nama: namaTrimmed,
						bungkus: Number.isFinite(bungkus) ? bungkus : 0,
					},
				];
			}
	...
	const getRowTotal = (row: Row) => {
		const b = Number.isFinite(row.bungkus) ? row.bungkus : 0;
		const upah = parseFloat(upahPerBungkus || "0");
		return b * upah;
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const res = await createPengemasan(payload);
			if (res?.success) {
	...
	const handleDownloadPerLembar = async () => {
		...
		rowsForPrint.forEach((row, idx) => {
			...
			if (logo) {
				...
				pdf.setFont("helvetica", "bold");
				pdf.setFontSize(9);
				pdf.text("NOTA PENGEMASAN", pageW - margin, lineY(y + 5), {
					align: "right",
				});
				...
				pdf.setFont("helvetica", "bold");
				pdf.setFontSize(12);
				pdf.text("NOTA PENGEMASAN", pageW / 2, lineY(y), { align: "center" });
				y += 8;
				pdf.setDrawColor(26, 35, 126);
				pdf.setLineWidth(0.4);
				pdf.line(margin, lineY(y), pageW - margin, lineY(y));
			...
			pdf.setFontSize(9);
			pdf.text(
				`Bal: ${row.bungkus.toLocaleString("id-ID")} buah`,
				margin,
				lineY(y)
			);
			y += 5;
			...
			const upah = parseFloat(upahPerBungkus || "0");
			pdf.text(
				`Upah per bungkus: Rp ${upah.toLocaleString("id-ID")}`,
				margin,
				lineY(y)
			);
			y += 7;
		...
		pdf.save(`nota-pengemasan-perlembar-${date || "draft"}.pdf`);
	};
	...
	const handleDownloadPdf = async () => {
		...
		if (logo) {
			...
			pdf.setFont("helvetica", "bold");
			pdf.setFontSize(9);
			pdf.text("REKAP PENGEMASAN", pageW - margin, y + 5, { align: "right" });
			...
			pdf.setFont("helvetica", "bold");
			pdf.setFontSize(12);
			pdf.text("REKAP PENGEMASAN", pageW / 2, y, { align: "center" });
			y += 8;
			pdf.setDrawColor(26, 35, 126);
			pdf.setLineWidth(0.4);
			pdf.line(margin, y, pageW - margin, y);
		...
		const colBungkusX = colNamaX + 60;
		const colTotalX = colBungkusX + 25;
		const rowHeight = 6;
		...
		pdf.text("Bungkus", colBungkusX, y);
		...
		rowsForPrint.forEach((row, idx) => {
			...
			pdf.text(row.bungkus.toLocaleString("id-ID"), colBungkusX, y, {
				align: "right",
			});
		...
		pdf.setFont("helvetica", "bold");
		pdf.text("Total Semua", colBungkusX, y);
		pdf.text(`Rp ${totalSemua.toLocaleString("id-ID")}`, colTotalX + 20, y, {
			align: "right",
		});

		pdf.save(`nota-pengemasan-rekap-${date || "draft"}.pdf`);
	};
	...
	return (
		<div className="space-y-4">
			<form onSubmit={handleSubmit}>
				<GlassCard className="p-4 md:p-6">
					<div className="flex items-start justify-between mb-4 gap-4">
						<div>
							<div className="text-lg font-semibold">Pengemasan</div>
							<div className="text-xs text-secondary">
								Input jumlah bungkus per pekerja
							</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs mt-3">
									...
									<div className="flex flex-col gap-1">
										...
										<input
											...
											min={0}
											value={newBungkus}
											onChange={(e) => setNewBungkus(e.target.value)}
										...
										>
											...
											<td className="px-3 py-2">
												<input
													type="number"
													min={0}
													value={row.bungkus === 0 ? "" : row.bungkus}
													onChange={(e) =>
														handleChange(row.id, "bungkus", e.target.value)
													}
			...
			<SafeModal
				open={openPreview}
				title="Preview Rekap Pengemasan"
				...
				<div className="flex justify-center">
					...
					>
						<div className="mb-3 flex items-center justify-between">
							<div className="font-semibold text-sm">Rekap Pengemasan</div>
						...
						<table className="w-full text-xs">
							<thead>
								<tr className="font-semibold">
									...
									<td className="py-1 text-right">Bungkus</td>
									...
									return (
										<tr key={row.id}>
											...
											<td className="py-1 text-right">
												{row.bungkus.toLocaleString("id-ID")}
											</td>
											<td className="py-1 text-right">
												Rp {total.toLocaleString("id-ID")}
											</td>
